...
class Dataloader():
    ...
    def reset(self):
        ...
        self.buffer_label_widebeam_m = np.zeros((0, 10))
    ...
    def load(self, file):
        ...
        labels_widebeam = data['max_id_sery_widebeam_m'] - 1
        labels_nonoise = data['max_id_sery_no_noise_m'] - 1
        beam_power_m = data['rsrp_sery_m']
        beam_power_nonoise_m = data['rsrp_sery_no_noise_m']
        return channel1, channel2, labels, beam_power_m, beam_power_nonoise_m, labels_nonoise, labels_widebeam
    ...
    def next_batch(self):
        ...
        while self.buffer1.shape[0] < self.batch_size:
            if len(self.unvisited_files) == 0:
                ...
                count = False
                break
            channel1, channel2, labels, beam_power_m, beam_power_nonoise_m, labels_nonoise, labels_widebeam = self.load(
                self.unvisited_files.pop(0))

            del self.buffer1
            del self.buffer2
            del self.buffer_label_m
            del self.buffer_label_widebeam_m
            ...
            self.buffer_label_widebeam_m = np.concatenate((self.buffer_label_widebeam_m, labels_widebeam), axis=0)
        ...
        batch_labels_widebeam_m = np.squeeze(self.buffer_label_widebeam_m[0:out_size, :])
        ...
        self.buffer_label_widebeam_m = np.delete(self.buffer_label_widebeam_m, np.s_[0:out_size], 0)
        ...
        batch_labels_widebeam_m = batch_labels_widebeam_m.astype(long)
        ...
        return torch.from_numpy(batch_channel1).to(self.device), torch.from_numpy(batch_channel2).to(
            ...
            self.device), torch.from_numpy(batch_labels_widebeam_m).to(
