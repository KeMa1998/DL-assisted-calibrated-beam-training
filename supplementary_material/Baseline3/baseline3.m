...
% BS antenna number
rx = 64;
resolution = 64; % targeted resolution
measure_t = 16; % number of each beam training
layer = log2(resolution);
...
%%
power_ratio = zeros(10, 4, 5);
pdf = zeros(101, 10, 4);
noise_e = [110; 112; 113; 113]; % Equivalent noise including AWGN and NLOS components
count = 0;

for snr_e = 115 : 5 : 130 % Equivalent SNR calculated as (P - sigma^2)
    count = count + 1;
    for n = 1 : 16
        file_name = ['..\dataset\data_TCOM(withLOSparameter)_16Tx_64Tx_RK8dB_' num2str(n) '.mat'];
        load(file_name);
        for i = 1 : 256
            for j = 1 : 10
...
%%
power_ratio_v = zeros(10, 5);
count = 0;

for v = 10 : 10 : 50 % Velocity
    count = count + 1;
    for n = 1 : 32
        file_name = ['..\dataset\data_TCOM(withLOSparameter)_16Tx_64Tx_RK8dB_' num2str(n) '_speed' num2str(v) '.mat'];
        load(file_name);
        for i = 1 : 256
            for j = 1 : 10
                mm_channel = squeeze(channel_m(i, j, :));
                ...
                rsrp = abs(beam_select);
                [~, max_rsrp_location] = max(rsrp);
                pis = 1 / resolution * ones(resolution, 1);
                for t = 1 : measure_t
                    k = 0;
                    l_star = 1;
                    for l = 1 : layer
                    ...
                    f = exp(- (abs(y - LOS_parameter * w * candidate_beam0.')).^2 / 2 / (10^(- 112 / 10)));
                ...
                [~, max_location] = max(pis);
                power_ratio_v(j, count) = power_ratio_v(j, count) + rsrp(max_location) ^ 2 / max(rsrp) ^ 2;
            end
        end
    end
end
...
%save('baseline3_evaluation.mat', 'power_ratio', 'pdf', 'power_ratio_v');